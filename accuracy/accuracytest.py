...
def test_recall_accuracy(suspected_node_list, source, file_name):
    """
    ...
    :return:
    """
    location_dict = load_location(file_name)
    ...
    for node in suspected_node_list:
        if node.private_info is None:
            node.private_info =[(word[0], purpose) for word in node.private_word_list for purpose in node.purpose]
            node.private_info = list(set(node.private_info))
...
def test_missed(suspected_node_list, source, file_name):
    location_dict = load_location(file_name)
...
def test_stamp(stamp, file_name):
    # print(stamp)
    data_type_compute = []
    purpose_compute = []
    for st in stamp:
    ...
    data_type_list, purpose_list = load_data_purpose_split(file_name)
